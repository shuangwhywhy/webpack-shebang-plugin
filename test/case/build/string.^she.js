...
const config = require('../configs/string.^she');
    ...
    describe('webpack ' + version, () => {
        ...
        it('shebang in place',  done => {
            for (const result of assets) {
                should.equal(/^#!\/usr\/bin\/env node\s+/.test(result.content), false);
            }
            done();
        });
        it('chmod ok',  done => {
            for (const result of assets) {
                should.not.equal(result.mode, '755');
            }
            done();
        });
    });
...
module.exports = () => {
    describe('entry JS has no shebang', () => {
